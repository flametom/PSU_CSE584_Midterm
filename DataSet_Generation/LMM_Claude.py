...
def generate_text_with_claude(prompt):
    try:
        response = client.messages.create(
            ...
            max_tokens=100,
            temperature=0.7,
            # system="You are a helpful assistant that completes sentences.",
            messages=[
...
def generate_text_with_detailed_prompt(x_i):
    prompt = (
    ...
    return generate_text_with_claude(prompt)

def complete_texts_batch(input_texts, max_length=100, batch_size=32, max_attempts=5):
    all_completed_texts = []
    ...
    for i in tqdm(range(0, len(input_texts), batch_size)):
        ...
        for attempt in range(max_attempts):
            ...
            for j, x_i in enumerate(current_batch):
                generated_text = generate_text_with_detailed_prompt(x_i)
                ...
                # Add a short wait time between API calls
                time.sleep(0.5)
...
batch_size = 128 # Adjust batch size considering Claude API usage
x_j_samples = complete_texts_batch(x_i_samples, max_length=100, batch_size=batch_size)
...
results.to_csv('X_i_samples_5k_Claude-3-haiku.csv', index=False)