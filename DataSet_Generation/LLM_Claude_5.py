...
def clean_text(text, x_i):
    ...
    # Select the first 5 sentences
    selected_sentences = sentences[:5]
    
    # Add a period if any sentence is incomplete (no punctuation at the end)
    selected_sentences = [sent.strip() + '.' if not sent.strip().endswith(('.', '!', '?')) else sent.strip() for sent in selected_sentences]
    
    # Remove duplicate words within each sentence (case-insensitive)
    cleaned_sentences = []
...
def is_valid_response(text, x_i):
    ...
    # Check if we have exactly 5 sentences
    if len(sentences) != 5:
        return False
    ...
    # Limit to minimum 15 words added (3 per sentence on average), maximum 250 words (50 per sentence on average)
    words = text.split()
    x_i_words = x_i.split()
    if len(words) - len(x_i_words) < 15 or len(words) > 250:
        return False
    
    # Check if the text contains an underscore ('_')
    if '_' in text:
        return False
    ...
    # Check if all sentences end with a period, question mark, or exclamation mark
...
def generate_text_with_claude(prompt, client):
    try:
        response = client.messages.create(
            ...
            max_tokens=500,  # Increased max_tokens to accommodate 5 sentences
            temperature=0.7,
            messages=[
...
def generate_text_with_detailed_prompt(x_i, client):
    prompt = (
        "Complete the following sentence by adding words to form five complete sentences, including the initial one. "
...
def complete_texts_batch(input_texts, client, batch_size=32, max_attempts=5):
    all_completed_texts = []
    ...
    for i in tqdm(range(0, len(input_texts), batch_size)):
        ...
        for attempt in range(max_attempts):
            ...
            for j, x_i in enumerate(current_batch):
                ...
                if is_valid_response(cleaned_text, x_i):
...
def generate_dataset(api_key, input_file, output_file, batch_size=32, max_rows=None):
    ...
    x_j_samples = complete_texts_batch(x_i_samples, client, batch_size=batch_size)
...
def run_claude_generation_5(api_key, input_file, output_file, batch_size=32, max_rows=None):
